...
A Go application that generates Go code that handles Create, Read, Update, Delete, and List/Search (CRUDL) operations against 
...
**go_dbmap** takes a very different approach to how client code should access and support a relational database backend. A vestige
...
well as the [DML (Data Manipulation Language)](https://en.wikipedia.org/wiki/Data_manipulation_language). It is the position of this author that this is a
...
the author can then follow proper modeling best-practices and create a schema that conforms to at minimum
...
Furthermore, once your database is in production, you will need to support any refactoring (i.e. ALTER) in a manner that does not
...
NOTE: depending on your dev environment, you might have to use your own username instead of `dbmap_test`
...
Run the tests again after you have generated the example code to then test the generated code `user_db` against the
`user` table to make sure everything works.

#### Building the Example
The project includes a fairly comprehensive test schema and scripts to build located in the `database` directory. You
...
### go_dbmap.yml
You will want to look at [config/go_dbmap.yml](config/go_dbmap.yml) as a guide
...
`go_dbmap` operates against database SQL operations, it only supports transformations between columns that 
can be legally defined by SQL. This implementation is not terribly sophisticated, it is important to understand 
...
exist in the table, they will be generated in the record map and returned on each read. This means than you 
